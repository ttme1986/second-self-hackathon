/**
 * E2E User Flow — Audio Conversation Test
 *
 * Identical to user-flow.spec.ts but uses the Gemini Live API with audio
 * instead of the text-only Chat API. A silent audio stream is fed via
 * Chromium's --use-fake-device-for-media-stream flag, simulating a user
 * with a microphone that captures silence. The assistant responds with
 * audio (played silently in headless mode) plus transcription.
 *
 * Flow:
 *   1. Login as demo user
 *   2. Navigate to /chat, trigger gesture to auto-start voice session
 *   3. Wait for auto-greeting from ChatAgent (via audio transcription)
 *   4. 10-turn text conversation (typed text sent via sendClientContent)
...
// Wait for assistant greeting/response to stabilize (longer timeouts for audio transcription)
async function waitForAssistantResponse(page: any, timeout = 45000): Promise<string> {
  const assistantArea = page.locator('.chat-center-assistant')
  await assistantArea.waitFor({ state: 'visible', timeout })

  // Audio transcription arrives in small chunks and may take longer to stabilize
  let prevText = ''
  let stableCount = 0
  for (let i = 0; i < 30; i++) {
    ...
    if (currentText === prevText && currentText.length > 0) {
      stableCount++
      if (stableCount >= 3) break // 3 consecutive stable checks for audio
    } else {
      stableCount = 0
    }
...
test.describe('User Flow E2E (Audio)', () => {
  test('10-turn audio conversation -> hang-up -> reflect validation -> second session', async ({ page, context }) => {
    ...
    page.on('console', (msg) => {
      ...
      if (text.includes('[LiveAPI]') || text.includes('[Chat]') || text.includes('Live API') || text.includes('Failed to send') || text.includes('auto-connect') || text.includes('voice session') || text.includes('memory context')) {
    ...
    // ============================================
    // 2. NAVIGATE: Go to /chat and trigger voice session
    // ============================================
    console.log('Step 2: Navigating to chat (audio mode)...')

    await page.goto('/chat')
    await page.waitForLoadState('domcontentloaded')
    await page.waitForTimeout(1000)

    // Trigger a gesture (pointerdown) to auto-start the voice session.
    // The app's useVoiceSession listens for the first pointerdown event
    // and then calls startVoiceSession() which requests mic access.
    // With --use-fake-device-for-media-stream, getUserMedia succeeds
    // and the Live API WebSocket connection is established.
    console.log('  Triggering gesture to start voice session...')
    await page.locator('body').click({ position: { x: 195, y: 700 } })
    await page.waitForTimeout(3000) // Wait for voice session to connect
    ...
    // ============================================
    // 2.5. WAIT FOR AUTO-GREETING (via audio transcription)
    // ============================================
    console.log('  Waiting for initial greeting (audio transcription)...')
    try {
      const greetingText = await waitForAssistantResponse(page, 45000)
      if (greetingText) {
    ...
    // ============================================
    // 3. CONVERSATION: 10-turn text chat loop (via Live API sendClientContent)
    // ============================================
    console.log('Step 3: Starting 10-turn conversation (text input, audio responses)...')
    ...
    for (let turn = 1; turn <= 10; turn++) {
      ...
      // Wait for assistant response (audio transcription)
      console.log(`  Turn ${turn}/10: Waiting for assistant response (audio)...`)

      try {
        const assistantText = await waitForAssistantResponse(page, 45000)
        if (assistantText) {
    ...
    // Trigger gesture for second voice session
    console.log('  Triggering gesture for second voice session...')
    await page.locator('body').click({ position: { x: 195, y: 700 } })
    ...
    try {
      secondGreetingText = await waitForAssistantResponse(page, 45000)
...
Reply ONLY with JSON (no markdown fences): {"correct": true/false, "explanation": "..."}`
    ...
    for (let turn = 1; turn <= 5; turn++) {
      ...
      console.log(`  Follow-up turn ${turn}/5: Waiting for assistant response (audio)...`)
...
Reply ONLY with JSON (no markdown fences): {"correct": true/false, "explanation": "..."}`
    ...
    console.log('User flow audio test complete!')
  })
})
